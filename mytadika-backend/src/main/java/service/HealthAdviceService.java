...
public class HealthAdviceService {
    ...
    public AdviceResult generateAdvice(
        ...
        // normal -> "1" (Normal), moderate -> "0" (Underweight), severe -> "0" (Underweight)
        ...
        if (status.equals("severe")) {
            // Always force include UW-04 (Consult health professional) as first recommendation
            AdviceCard urgentCard = dietaryPool.stream().filter(c -> c.getId().equals("UW-04")).findFirst().orElse(null);
            List<AdviceCard> remainingPool = dietaryPool.stream().filter(c -> !c.getId().equals("UW-04")).collect(Collectors.toList());
        ...
        List<AdviceCard> activityPool = templates.getActivityAdviceDatabase().getOrDefault(String.valueOf(activityLevel), Collections.emptyList());
        List<AdviceCard> selectedActivity = selectAdvice(activityPool, 3, normalizedAllergies, shownAdviceIds, guardrailLog);
    ...
    private List<AdviceCard> selectAdvice(
        ...
        List<AdviceCard> fresh = allowed.stream().filter(c -> !shownAdviceIds.contains(c.getId())).collect(Collectors.toList());
        List<AdviceCard> stale = allowed.stream().filter(c -> shownAdviceIds.contains(c.getId())).collect(Collectors.toList());
